...
public interface AccessPermissionRepository extends JpaRepository<AccessPermission, String> {
    ...
    @Query("SELECT DISTINCT service.id FROM AccessPermission permission "
            + "JOIN permission.inboundEndpoint endpoint "
            + "JOIN endpoint.secureService service "
            + "WHERE permission.client.id = :clientId")
    ...
    @Query("SELECT permission FROM AccessPermission permission "
            ...
            + "AND (:keywordPattern IS NULL OR LOWER(service.id) LIKE :keywordPattern "
            + "OR LOWER(endpoint.id) LIKE :keywordPattern "
            + "OR LOWER(client.id) LIKE :keywordPattern "
            + "OR LOWER(client.name) LIKE :keywordPattern "
